...
gencode_get_transcripts <-
    function(build = "grch38",
        ...
        #
        lgr::lgr$info(paste0("A total of ", nrow(gencode), 
                             " transcripts parsed"))
        ...
        if (append_regulatory_region == TRUE) {
            ...
            lgr::lgr$info(
                paste0("Parameter 'append_regulatory_region' is TRUE: ",
            ...
            chromosome_lengths <- data.frame(
                ...
                "chrom_length" = head(
                    GenomeInfoDb::seqlengths(
                        BSgenome.Hsapiens.UCSC.hg19), 25),
            ...
            if (build == "grch38") {
                chromosome_lengths <- data.frame(
                    "chrom" = head(
                        names(
                            GenomeInfoDb::seqlengths(
                                BSgenome.Hsapiens.UCSC.hg38)), 25),
                    "chrom_length" = head(
                        GenomeInfoDb::seqlengths(
                            BSgenome.Hsapiens.UCSC.hg38), 25),
                    stringsAsFactors = FALSE, row.names = NULL
        ...
        lgr::lgr$info(paste0("A total of ", 
...
get_uniprot_map <- function(uniprot_version = "2022_03") {
