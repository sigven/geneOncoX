  ...
  function(gene_info = NULL,
    ...
    while(i <= nrow(panels)){
      ...
      if(build == "grch38"){
        df <- data.frame(
          ...
          'gepa_moi' = contents$genes$mode_of_inheritance,
          'gepa_penetrance' = contents$genes$penetrance,
          'gepa_confidence_level' = as.integer(contents$genes$confidence_level),
          'gepa_panel_name' = contents$name,
          'gepa_panel_id' = contents$id,
          'gepa_panel_version' = contents$version,
      ...
      df$id <- i
      df$gepa_panel_url <- paste0('https://panelapp.genomicsengland.co.uk/panels/',df$gepa_panel_id)
    ...
    if(build == "grch38"){
      repair_genes_panel <- all_repair_genes |>
        ...
        dplyr::mutate(id = i, gepa_moi = NA, gepa_panel_id = 256,
                      gepa_panel_version = as.character(version),
                      gepa_penetrance = NA,
                      gepa_phenotype = NA,
                      gepa_panel_url = "https://panelapp.genomicsengland.co.uk/panels/256/",
                      gepa_confidence_level = 0,
                      hgnc_id = stringr::str_replace(hgnc,'HGNC:',''),
                      ensembl_gene_id = ensembl_id_g_rch38) |>
        dplyr::select(id, ensembl_gene_id, hgnc_id,
      ...
      repair_genes_panel <- all_repair_genes |>
        ...
        dplyr::select(id, ensembl_gene_id, hgnc_id, gepa_phenotype,
                      gepa_panel_name, gepa_moi, gepa_panel_id,
                      gepa_penetrance, gepa_panel_version,
                      gepa_panel_url, gepa_confidence_level)
    ...
    all_panels <- all_panels |>
      ...
      dplyr::mutate(
        gepa_moi2 = dplyr::if_else(
          stringr::str_detect(gepa_moi,"MONOALLELIC, autosomal or pseudoautosomal, NOT imprinted") |
            stringr::str_detect(gepa_moi,"MONOALLELIC, autosomal or pseudoautosomal, imprinted status unknown"),
          "AD",as.character(NA))) |>
      dplyr::mutate(gepa_moi2 = dplyr::if_else(
        stringr::str_detect(gepa_moi,"BIALLELIC, autosomal or pseudoautosomal"),
      ...
      #dplyr::select(-gepa_moi) |>
      dplyr::left_join(
        dplyr::select(gene_info, hgnc_id,
                      entrezgene, name),by="hgnc_id") |>
      dplyr::rename(genename = name) |>
      dplyr::select(-hgnc_id) |>
      dplyr::filter(!is.na(entrezgene)) |>
      dplyr::select(genome_build, id, entrezgene, genename, ensembl_gene_id,
                    dplyr::everything())
    ...
    #     dplyr::summarise(gepa_panel = paste(tmp, collapse="&"),
    #                      gepa_moi = paste(unique(sort(gepa_moi)), collapse=","),
    #                      gepa_phenotype = paste(unique(gepa_phenotype), collapse=", "),
    #                      .groups = "drop") |>
    #     dplyr::select(entrezgene, gepa_phenotype, gepa_moi, gepa_panel) |>
    #     dplyr::mutate(gepa_phenotype = stringr::str_replace_all(gepa_phenotype,"^(\\s{0,},\\s{1,})",""))
    # )
    #
    # ret <- list('wide' = panel_df,
    #             'long' = all_panels)
    #
    # return(ret)
...
get_acmg_secondary_findings <- function(gene_info = NULL){
  ...
  acmg_sf_list <- as.data.frame(
    openxlsx::read.xlsx(file.path(
      ...
      sheet = 1,startRow = 3) |>
    janitor::clean_names() |>
    dplyr::rename(symbol = gene, disease_phenotype = disease_phentyope) |>
    ...
    dplyr::mutate(variants_to_report = stringr::str_trim(variants_to_report)) |>
    dplyr::mutate(disease_phenotype = dplyr::if_else(
      stringr::str_detect(disease_phenotype,"Long-QT"),
      stringr::str_replace_all(disease_phenotype," type |-"," "),
      as.character(disease_phenotype)
    )) |>
    dplyr::mutate(disease_phenotype = dplyr::if_else(
      stringr::str_detect(disease_phenotype,"Familial medullary"),
      stringr::str_replace_all(disease_phenotype," cancer"," carcinoma"),
      as.character(disease_phenotype)
    )) |>
    dplyr::mutate(disease_phenotype = dplyr::if_else(
    ...
    dplyr::mutate(cui = dplyr::if_else(
      disease_phenotype == "Hereditary paraganglioma-pheochromocytoma syndrome",
      "C1708353",
      as.character(cui)
    )) |>
    dplyr::distinct() |>
    dplyr::group_by_at(dplyr::vars(-cui)) |>
    dplyr::summarise(cui = paste(cui, collapse=", "),
                     .groups = "drop") |>
    #dplyr::rename(predisp_syndrome_cui = cui) |>
    dplyr::left_join(
      dplyr::select(gene_info, symbol, entrezgene),by=c("symbol")) |>
    dplyr::group_by(entrezgene, gene_mim, variants_to_report, inheritance, sf_list_version) |>
    dplyr::summarise(cui = paste(sort(unique(cui)), collapse=", "),
                     disease_phenotype = paste(sort(unique(disease_phenotype)), collapse=";"),
                     disorder_mim = paste(sort(unique(disorder_mim)), collapse=";"),
                     .groups = "drop") |>
...
get_predisposition_genes_huang018 <- function(gene_info = NULL){
  ...
  tcga_pancan2018_genes <-
    openxlsx::read.xlsx(
      ...
      sheet = 1) |>
    janitor::clean_names() |>
    dplyr::select(gene_symbol, mode_of_inheritance, major_associated_tumor_types, cancer_syndrome_s) |>
    dplyr::rename(symbol = gene_symbol, tumor_type = major_associated_tumor_types, cancer_syndrome = cancer_syndrome_s) |>
    dplyr::mutate(mode_of_inheritance = stringr::str_replace(stringr::str_trim(mode_of_inheritance),'\n',''),
                  symbol = stringr::str_trim(symbol)) |>
    dplyr::mutate(tumor_type = stringr::str_replace(stringr::str_trim(tumor_type),"(Biallelic|Monoallelic) mutations:","")) |>
    dplyr::mutate(tumor_type = stringr::str_replace_all(stringr::str_to_title(tumor_type),"\\s?\\r\\n",", ")) |>
    dplyr::mutate(tumor_type = stringr::str_replace(tumor_type,"^\\s{0,},\\s{0,}","")) |>
    dplyr::mutate(tumor_type = stringr::str_trim(tumor_type)) |>
    dplyr::mutate(tumor_type = stringr::str_replace(tumor_type,"Mds","MDS")) |>
    dplyr::mutate(tumor_type = stringr::str_replace(tumor_type,"(-| )All"," ALL")) |>
    dplyr::mutate(tumor_type = stringr::str_replace(tumor_type,"Bcc","BCC")) |>
    dplyr::mutate(tumor_type = stringr::str_replace(tumor_type,"Mpn","MPN")) |>
    dplyr::mutate(tumor_type = stringr::str_replace(tumor_type,"Jmml","JMML")) |>
    dplyr::mutate(tumor_type = stringr::str_replace(tumor_type,"Aml","AML")) |>
    dplyr::mutate(moi = dplyr::if_else(
      stringr::str_detect(mode_of_inheritance,"autosomal dominant"),"AD",as.character(NA))) |>
    dplyr::mutate(moi = dplyr::if_else(
      stringr::str_detect(mode_of_inheritance,"autosomal recessive"),"AR",as.character(moi))) |>
    dplyr::mutate(moi = dplyr::if_else(
      stringr::str_detect(mode_of_inheritance,"autosomal dominant") &
        stringr::str_detect(mode_of_inheritance,"autosomal recessive"),"AD/AR",as.character(moi))) |>
  ...
  tcga_2018_predisposition_umls <-
    openxlsx::read.xlsx(
      file.path(
        ...
        "huang_pancancer_cell_2018.xlsx"),
      sheet = 4) |>
    dplyr::mutate(predisp_syndrome_cui = stringr::str_trim(cui_syndrome),
                  susceptibility_cui = stringr::str_trim(cui_susceptibility)) |>
    dplyr::mutate(susceptibility_cui = stringr::str_replace_all(susceptibility_cui,";","&")) |>
    dplyr::mutate(predisp_syndrome_cui = stringr::str_replace_all(predisp_syndrome_cui,";","&")) |>
...
get_curated_predisposition_genes <- function(gene_info = NULL){
  ...
  curated_predisposition_genes <- as.data.frame(
    curated_other |>
      ...
      dplyr::summarise(moi = paste(unique(moi), collapse=","),
...
get_moi_mod_maxwell2016 <- function(gene_info = NULL){
  ...
  mod_moi_predisposition <-
    openxlsx::read.xlsx(
      ...
      sheet = 1, startRow = 2) |>
    janitor::clean_names() |>
    dplyr::filter(!stringr::str_detect(gene_name, "^Autosomal|Misc|Genes")) |>
    dplyr::mutate(symbol = gene_name,
  ...
  cpg_collections[['PANEL_APP']] <- as.data.frame(
    gene_panels$records |>
      ...
      dplyr::summarise(
        ...
        phenotypes = paste(unique(sort(gepa_phenotype)), collapse=";"),
        .groups = "drop") |>
  ...
  all_predisposition_incidental <- as.data.frame(
    cpg_collections[["CGC"]] |>
      ...
      dplyr::summarise(
        ...
        predisp_syndrome_cui = paste(unique(predisp_syndrome_cui),collapse="&"),
        susceptibility_cui = paste(unique(susceptibility_cui),collapse="&"),
