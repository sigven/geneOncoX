        ...
        all_panels <- all_panels |>
            ...
            dplyr::mutate(
                gepa_moi2 = dplyr::if_else(
                    stringr::str_detect(
                        gepa_moi,
                        paste0("MONOALLELIC, autosomal or ",
    ...
    tcga_pancan2018_genes <-
        ...
        dplyr::mutate(moi = dplyr::if_else(
            stringr::str_detect(
                ...
                stringr::str_detect(
                    mode_of_inheritance, 
                    "autosomal recessive"), "AD/AR", 
            as.character(moi)
        )) |>
        dplyr::mutate(phenotypes = paste(
            tumor_type, cancer_syndrome, sep = "; ")) |>
        dplyr::select(
    ...
    cpg_collections[["PANEL_APP"]] <- as.data.frame(
        gene_panels$records |>
            ...
            dplyr::summarise(
                ...
                phenotypes = paste(
                    unique(sort(gepa_phenotype)), collapse = ";"),
                .groups = "drop"
            ) |>
        ...
        cpg_collections[["CGC"]] |>
            ...
            dplyr::summarise(
                ...
                predisp_syndrome_cui = paste(
                    unique(predisp_syndrome_cui), collapse = "&"),
                susceptibility_cui = paste(
                    unique(susceptibility_cui), collapse = "&"),
